...
@SpringBootTest(properties = {
        "spring.datasource.url=jdbc:h2:mem:demo_seed_disabled;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;DEFAULT_NULL_ORDERING=HIGH;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE",
...
@ActiveProfiles("dev")
class DemoDataSeederDisabledIntegrationTest {
    ...
    @Test
    void shouldNotCreateDemoDataWhenDisabled() {
        assertThat(clientRepository.findAll()).isEmpty();
        assertThat(vehicleRepository.findAll()).isEmpty();
        assertThat(loanOperationRepository.findAll()).isEmpty();
    }
}
